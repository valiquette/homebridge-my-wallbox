'use strict'
let wallboxAPI = require('homebridge-my-wallbox/src/wallboxapi').default
...
class wallboxPlatform {
	constructor(log, config, api) {
		...
		this.lockMechanism = new lockMechanism(this, log, api)
		this.battery = new battery(this, log, api)
		this.sensor = new sensor(this, log, api)
		this.basicSwitch = new basicSwitch(this, log, config, api)
		this.outlet = new outlet(this, log, config, api)
		this.control = new control(this, log, config, api)
	...
	async getDevices() {
		try {
			...
			//get me
			let me = await this.wallboxapi.me(this.token).catch(err => {
			...
			groups.result.groups
				...
				.forEach(group => {
					...
					group.chargers.forEach(async charger => {
						...
						//loop each charger
						let chargerData = await this.wallboxapi.getChargerData(this.token, charger.id).catch(err => {
							this.log.error('Failed to get charger data for build. \n%s', err)
						})
						let chargerConfig = await this.wallboxapi.getChargerConfig(this.token, charger.id).catch(err => {
							this.log.error('Failed to get charger configs for build. \n%s', err)
						})

						let uuid = genUUID(chargerData.uid)
	...
	async updateStatus(charger) {
		try {
			...
			if (this.showBattery) {
				...
				if (this.showSensor) {
					sensorService = lockAccessory.getServiceById(Service.HumiditySensor, chargerID)
				}
			}
			if (this.showControls == 1 || this.showControls == 4) {
				switchService = lockAccessory.getServiceById(Service.Switch, chargerID)
			}
			if (this.showControls == 5 || this.showControls == 4) {
				outletService = lockAccessory.getServiceById(Service.Outlet, chargerID)
			}
			if (this.showControls == 3 || this.showControls == 4) {
				controlService = lockAccessory.getServiceById(Service.Thermostat, chargerID)
			}
			/****
			...
			****/
			...
			switch (statusInfo.mode) {
				...
				case 'readyMode':
					...
					if (this.showControls == 5 || this.showControls == 4) {
						outletService.getCharacteristic(Characteristic.On).updateValue(false)
					}
					if (this.showControls == 3 || this.showControls == 4) {
						...
						if (this.useFahrenheit) {
							controlService.getCharacteristic(Characteristic.CurrentTemperature).updateValue((((maxAmps - 32 + 0.01) * 5) / 9).toFixed(2))
							controlService.getCharacteristic(Characteristic.TargetTemperature).updateValue((((maxAmps - 32 + 0.01) * 5) / 9).toFixed(2))
						} else {
					...
					if (this.showBattery) {
						...
						if (this.showSensor) {
							sensorService.getCharacteristic(Characteristic.CurrentRelativeHumidity).updateValue(batteryPercent)
						}
					}
					break
				case 'chargingMode':
					...
					if (this.showControls == 5 || this.showControls == 4) {
						outletService.getCharacteristic(Characteristic.On).updateValue(true)
					}
					if (this.showControls == 3 || this.showControls == 4) {
